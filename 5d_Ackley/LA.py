...
class WEI(AnalyticAcquisitionFunction):
    ...
    def forward(self, X: Tensor) -> Tensor:
        ...
        dist = (unnormalize(X, bounds=bounds) - unnormalize(self.current_loc, bounds=bounds)).norm(dim=-1).reshape(sigma.shape)
        return sigma * _ei_helper(u) - dist / Normalize_y
...
if __name__ == "__main__":
    ...
    pd.DataFrame(cum_regret_table).T.to_csv("LA_reg.csv", index=False)
    pd.DataFrame(cum_travel_table).T.to_csv("LA_travel.csv", index=False)