...
class agent():
    ...
    def plan_next_batch(self): 
        ...
        for _ in range(int(self.current_batch_size)):
            ...
            X_cand = X_cand[feasiblity(X_cand, self.model_hty, self.LCB_hty)]
            if X_cand.numel() < 1000:
                feas_points = get_feasible_points(1000-X_cand.numel(), self.model_hty, self.LCB_hty, self.BOD)
...
if __name__ == "__main__":
    ...
    pd.DataFrame(cum_regret_table).T.to_csv("TTS_reg.csv", index=False)
    pd.DataFrame(cum_travel_table).T.to_csv("TTS_travel.csv", index=False)