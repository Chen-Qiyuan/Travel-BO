...
class agent():
    ...
    def go(self):
        ...
        self.previous_batch_size = self.next_samples.shape[0]
    ...
    def plan_next_batch(self):
        ...
        _ , sigma = compute_mean_and_sigma(self.model, candidates)

        B_h = int(min(torch.ceil((C**2-1)/sigma**2).item(), T - (self.local_X.shape[0] - initial_samples)))
        
        # observe new values 
        self.next_samples = candidates.repeat(B_h,1)
...
if __name__ == "__main__":
    ...
    pd.DataFrame(cum_regret_table).T.to_csv("MINI_reg.csv", index=False)
    pd.DataFrame(cum_travel_table).T.to_csv("MINI_travel.csv", index=False)