...
from botorch.test_functions import Hartmann, Cosine8, Levy, Ackley, Branin, DropWave, Beale, EggHolder, Shekel, Powell, Griewank
...
class agent():
    ...
    def report(self,):
        ...
        self.plot_route(self.previous_batch_size)
    ...
    def plot_route(self, starts = 0):
        if starts ==0:
            name = "all"
        else:
            name = str(self.local_X.shape[0])
        # Create the heatmap
        resolution = 10000
        x = np.linspace(0, 1, resolution)
        y = np.linspace(0, 1, resolution)
        x, y = np.meshgrid(x, y)
        Z = blackbox.evaluate_true(unnormalize(torch.tensor(np.concatenate([x[:,:,None],y[:,:,None]],axis=-1)),bounds.cpu()))
        # plt.imshow(Z, cmap='coolwarm', norm=PowerNorm(gamma=0.3))
        plt.imshow(Z, cmap='coolwarm')
        
        # Plot the points
        x, y =  zip(*self.local_X[-starts:].cpu().numpy()*resolution)
        plt.plot(x, y, marker='o', markersize = 5, linewidth = 2, color = "red")
        plt.axis('off')
        plt.savefig("TTS"+name+".jpg", dpi = 600, bbox_inches='tight', pad_inches=0)
        plt.clf()
        # plt.show()
