...
def feasiblity(X, model_hty, LCB_hty, max_count = 5):
    indicator = torch.ones(X.shape[0])
    n_constraints = min(max_count, len(model_hty))
    for count in range(1,n_constraints+1):
        ...
        lcb = LCB_hty[-count]
        indicator *= (ucb>lcb)
    return indicator.type(torch.bool)

def get_feasible_points(N, model_hty, LCB_hty, BOD):
    X = torch.tensor([])

    Xraw = torch.rand(10*N, dim)
    X_pos = Xraw[feasiblity(Xraw, model_hty, LCB_hty)]
    X = torch.cat([X, X_pos])[:N]

    if X.shape[0] < N:
        Xraw = (BOD + (torch.rand(N,dim)-0.5) * 0.1).clamp(0,1)
        X_pos = Xraw[feasiblity(Xraw, model_hty, LCB_hty)]
        X = torch.cat([X, X_pos])[:N]
...
class ConstrainedUpperConfidenceBound(AnalyticAcquisitionFunction):
    ...
    def forward(self, X: Tensor) -> Tensor:
        ...
        return (mean + self.beta.sqrt() * sigma.squeeze()) + feasiblity(X, self.model_hty, self.LCB_hty).type(torch.double)
...
class agent():
    ...
    def plan_next_batch(self):
        ...
        for batch_index in range(int(self.current_batch_size)):
            feas_points = get_feasible_points(RAW_SAMPLES, self.model_hty, self.LCB_hty, self.BOD)
    ...
    def eval_BOD(self):
        ...
        self.BOD = candidate # for numerical stability

        return (blackbox.evaluate_true(unnormalize(self.BOD, bounds)) - blackbox.evaluate_true(blackbox.optimizers[0]) ).abs().item()
...
if __name__ == "__main__":
    ...
    # Setting parameters
    NOISE_SE = 0.01
    blackbox = DropWave(negate=True,noise_std = NOISE_SE)
    dim = blackbox.dim
    bounds = torch.tensor([[-5.12] * dim, [5.12] * dim])
    ...
    batch_size_multiplier = 2 # next batch size = multiplier * previous batch size
    ...
    pd.DataFrame(cum_regret_table).T.to_csv("TUCB2_reg.csv", index=False)
    pd.DataFrame(cum_travel_table).T.to_csv("TUCB2_travel.csv", index=False)