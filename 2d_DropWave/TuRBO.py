...
class agent():
    ...
    def plan_next_batch(self):
        if self.state.restart_triggered:
            self.next_samples = torch.rand(2*dim,dim)
            self.local_X = torch.tensor([])
            self.local_Y = torch.tensor([])
            ...
            X_cand = unnormalize(sobol.draw(1000), torch.stack([tr_lb, tr_ub]))
            # Thompson sample
...
if __name__ == "__main__":
    ...
    pd.DataFrame(cum_regret_table).T.to_csv("TuRBO_reg.csv", index=False)
    pd.DataFrame(cum_travel_table).T.to_csv("TuRBO_travel.csv", index=False)