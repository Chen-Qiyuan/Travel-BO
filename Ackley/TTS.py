...
from botorch.test_functions import Hartmann, Cosine8, Levy, Ackley, Branin, DropWave,Shekel, EggHolder, Powell, Griewank
...
class Feasibility(AnalyticAcquisitionFunction):
    ...
    def forward(self, X: Tensor) -> Tensor:
        feasiblity = compute_feasiblity_indicator(X, self.model_history, self.LCB_history, self.beta, scale=1000)
        return  feasiblity 
...
class agent():
    ...
    def go(self):
        ...
        regret = (blackbox.evaluate_true(unnormalize(self.next_samples, bounds)) - blackbox.optimal_value).abs().sum()
    ...
    def plan_next_batch(self): 
        ...
        for _ in range(int(self.current_batch_size)):
            ...
            feasiblity = compute_feasiblity_indicator(X_cand, self.model_history, self.LCB_history, beta, scale=1000)
            X_cand = X_cand[feasiblity==1]
            if X_cand.numel() < 10:
                ...
                while X_cand.numel() < 10:
                    ...
                    X_cand = torch.cat([X_cand, cand.detach()])
            ...
            print("\rSamples Planed: {}".format(_), end="")
