...
from botorch.test_functions import Hartmann, Cosine8, Levy, Ackley, Branin, DropWave, EggHolder
...
class agent():
    ...
    def update_model(self):      
        ...
        # self.model = SingleTaskGP(self.local_X, self.local_Y.unsqueeze(-1)).cuda()
    ...
    def plan_next_batch(self):         
        ...
        candidates_before_delete = torch.tensor([]).cuda().double()
        for _ in range(T):
            X_cand = sobol.draw(100).cuda().double()
            # Thompson sample
        ...
        candidates_after = self.point_delete(candidates_before_delete)
        next_index = (candidates_after - self.local_X[-1]).norm(dim=-1).argmin()
        self.next_samples = candidates_after[next_index].unsqueeze(0)
        self.cumulative_traveling_cost += (unnormalize(candidates_after[next_index],bounds) - unnormalize(self.local_X[-1],bounds)).norm(dim=-1).item()
    
    def point_delete(self,cand):
        eps = self.model.covar_module.base_kernel.lengthscale.min()
    ...
    def report(self,):
        if self.local_X.shape[0] == self.total_size + self.current_batch_size or self.local_X.shape[0] == T:
            ...
            # self.current_batch_size = round((self.previous_batch_size*T)**0.5) # set next batch size
            self.previous_batch_size = self.current_batch_size + 0
...
if __name__ == "__main__":
    ...
    NOISE_SE = 1
    negate = -1
    blackbox = Ackley(negate=True,noise_std = NOISE_SE, dim=2)
    dim = blackbox.dim
    ...
    import pandas as pd
    pd.DataFrame(cum_regret_table).T.to_excel("snake_reg.xlsx", index=False, engine='openpyxl')
