...
if __name__ == "__main__":
    ...
    # Setting parameters
    NOISE_SE = 0.1
    blackbox = Levy(negate=True,noise_std = NOISE_SE, dim = 4).to(device)
    dim = blackbox.dim
    bounds = torch.tensor([[-3] * dim, [3] * dim]).double().to(device)
    ...
    import pandas as pd
    pd.DataFrame(cum_regret_table).T.to_excel("MINI_reg.xlsx", index=False, engine='openpyxl')
    pd.DataFrame(cum_travel_table).T.to_excel("MINI_travel.xlsx", index=False, engine='openpyxl')