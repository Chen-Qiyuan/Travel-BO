...
class agent():
    ...
    def plan_next_batch(self):
        self.next_samples = torch.rand(1,dim).double().to(device)
...
if __name__ == "__main__":
    ...
    import pandas as pd
    pd.DataFrame(cum_regret_table).T.to_excel("RS_reg.xlsx", index=False, engine='openpyxl')
    pd.DataFrame(cum_travel_table).T.to_excel("RS_travel.xlsx", index=False, engine='openpyxl')